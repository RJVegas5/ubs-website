...
const services = [
  {
    ...
    category: "Cleaning",
    slug: "/services/commercial-janitorial",
  ...
  },
  {
    ...
    slug: "/services/building-maintenance",
  ...
  },
  {
    ...
    category: "Specialty",
    slug: "/services/electrostatic-disinfection",
  ...
  },
  {
    ...
    category: "Specialty",
    slug: "/services/post-construction-cleanup",
    desc: "Thorough commercial post-construction and renovation cleanup. From rough clean to final polish — we prepare your facility for opening day so you can move in with confidence.",
    details: ["Construction debris removal", "Dust & drywall dust removal", "Window & glass cleaning", "Paint overspray removal", "Sticker & adhesive removal", "Final inspection & touch-ups"],
    icon: (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" className="w-7 h-7"><rect x="2" y="7" width="20" height="14" rx="2" ry="2"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></svg>),
  },
  {
    num: "10",
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-wrap gap-2 mb-10">
          {CATEGORIES.map((cat) => (
            <button
              ...
              className="font-cond font-bold text-xs tracking-widest uppercase px-4 py-2 transition-all duration-200"
              style={{
                background:
                  activeCategory === cat
                    ? CATEGORY_COLORS[cat]
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/5 mb-16">
          <AnimatePresence mode="popLayout">
            {filtered.map((svc, i) => {
              const color = CATEGORY_COLORS[svc.category] ?? "#F5C518";
              return (
                <motion.div
                  key={svc.num}
                  layout
                ...
                >
                  ...
                  <div className="absolute top-4 right-4 font-display text-[80px] leading-none text-white/[0.025] select-none">
                    {svc.num}
                  </div>

                  {/* Category badge */}
                  <div
                    className="inline-block px-2 py-0.5 mb-4"
                    style={{
                      background: `${color}18`,
                  ...
                  >
                    <span
                      className="font-cond text-[10px] tracking-widest uppercase"
                      style={{ color }}
                    >
                      {svc.category}
                    </span>
                  ...
                  <h3 className="font-cond font-bold text-lg tracking-wider uppercase text-white mb-3">
                    {svc.name}
                  </h3>
                  ...
                  <ul className="space-y-1.5 mb-6">
                    {svc.details.map((d) => (
                      <li key={d} className="flex items-center gap-2 text-white/35 text-xs font-light">
                        <div className="w-1 h-1 rounded-full flex-shrink-0" style={{ background: `${color}80` }} />
                        {d}
                      </li>
                    ))}
                  </ul>
                  ...
                  <div className="flex items-center gap-3">
                    {svc.slug && (
                      <Link
                        href={svc.slug}
                        className="inline-flex items-center gap-1.5 font-cond font-semibold text-xs tracking-widest uppercase transition-all duration-200"
                        style={{ color }}
                      >
                        Learn More
                    ...
                    <Link
                      href="/contact"
                      className="inline-flex items-center gap-1.5 font-cond font-semibold text-xs tracking-widest uppercase text-white/40 hover:text-[#F5C518] transition-colors duration-200"
                    >
                      {svc.slug ? "Get Quote" : "Get a Quote →"}
                    </Link>
                  </div>
                </motion.div>
              );
        ...
        >
          ...
          <p className="text-white/50 font-light mb-8 max-w-lg mx-auto">
            Most clients use us for 3+ services. We bundle them together for a
            custom quote that works for your budget and schedule.
          </p>
          <Link
            href="/contact"
            className="inline-block font-cond font-bold text-sm tracking-widest uppercase bg-[#F5C518] text-[#0D0F1E] px-10 py-4 hover:bg-[#D4A800] transition-colors"
          >
